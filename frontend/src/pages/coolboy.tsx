...
export default function Home() {
  const [nowImg, setNowImg] = useState<string>('/demo/ikemen-normal.svg')
  ...
  const stopListening = () => {
    ...
    setHukidashiText(
      '今日は友達とちょっと早いけど海に行ったんだよ そしたら 夕方の海の景色が超綺麗だった'
    )
    setIsHukidashi(1)
    setTimeout(() => {
      playAudio()
      setNowImg('/demo/ikemen-smile.svg')
    }, 2000)
  }
  const playPlayer = () => {
    const audioContext = new AudioContext()
    const audioElement = new Audio('/audio/user.m4a')
  ...
  const playAudio = () => {
    const text =
      '海に行ったんだね。素敵な時間を過ごせたんだろう。夕方の海の景色って本当に美しいよね。どんな感じだったか詳しく教えてくれる？'
    let index = 0
    ...
    setIsHukidashi(2)
    const audioContext = new AudioContext()
    const audioElement = new Audio('/audio/ikemen.wav')
    const audioSource = audioContext.createMediaElementSource(audioElement)
    audioSource.connect(audioContext.destination)
    audioElement.play()
    setTimeout(() => {
  ...
  return (
    <div className={styles.grid}>
      ...
      {isHukidashi === 1 ? (
        <>
          <div className={styles.hukidashiArea}></div>
          <Image
      ...
      <div className={styles.topArea}>
        <div className={styles.topLeftArea}>
          ...
          <div className={styles.nameObj}>
            <p className={styles.nameText}>宝瓶蒼月</p>
          </div>
